...
const TTL_PUSHBOX_UPDATE = 1 * 3600 // 1 hour
...
module.exports = function (log, db, config) {
  ...
  return {
    ...
    /**
     * Notify devices that a new pushbox message is ready to be retrieved
     *
     * @param {String} uid
     * @param {Device} device to notify
     * @param {String} reason
     * @param {String} Pushbox index token
     * @promise
     */
    notifyPushboxUpdated(uid, device, service, reason, index) {
      const options = {
        data: {
          service,
          index
        },
        TTL: TTL_PUSHBOX_UPDATE
      }
